...
Some modules under `main_code/` require the SE gate implementation from [`ai-med/squeeze_and_excitation`](https://github.com/ai-med/squeeze_and_excitation). This repository does not redistribute that external code. Before running SE-gate-based models, download it into `main_code/`:

```bash
cd /path/to/project/root/PR_Review/code_release/sci_rds/main_code
git clone https://github.com/ai-med/squeeze_and_excitation.git
```

After downloading, the expected path is:

```text
main_code/squeeze_and_excitation/
```
